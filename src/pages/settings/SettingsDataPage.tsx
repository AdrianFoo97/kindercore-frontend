...
import { faBullhorn, faGraduationCap, faUserPlus } from '@fortawesome/free-solid-svg-icons';
import ImportLeadsPage from '../ImportLeadsPage.js';
import ImportStudentsPage from '../ImportStudentsPage.js';
import ImportCandidatesPage from '../candidates/ImportCandidatesPage.js';

const C = {
  primary: '#5a67d8', primaryLight: '#eef0fa', text: '#1e293b', muted: '#94a3b8', border: '#e2e8f0',
};

type TabKey = 'leads' | 'students' | 'candidates';

const TABS: { key: TabKey; label: string; icon: any }[] = [
  { key: 'leads', label: 'Import Leads', icon: faBullhorn },
  { key: 'students', label: 'Import Students', icon: faGraduationCap },
  { key: 'candidates', label: 'Import Candidates', icon: faUserPlus },
];
...
export default function SettingsDataPage() {
  ...
  return (
    <div style={s.page}>
      <style>{`.data-settings-tab:hover { color: ${C.text} !important; background: #f1f5f9 !important; }`}</style>
      <div style={s.inner}>
        ...
        <div style={s.content}>
          {tab === 'leads' && <ImportLeadsPage embedded />}
          {tab === 'students' && <ImportStudentsPage embedded />}
          {tab === 'candidates' && <ImportCandidatesPage embedded />}
        </div>
      </div>
    </div>
  );
